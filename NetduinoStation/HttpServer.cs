	...
	public class HttpServer
	{
		...
		private void ProcessRequest()
		{
			...
			else if (FILE_NAME.Equals("settings.json"))
			{
				...
				hashtable.Add("NTPServer", updater.NistServerAddress);
				hashtable.Add("TimeInterval", updater.TimeUpdateInterval);
				hashtable.Add("SensorInterval", updater.WeatherUpdateInterval);
				string json = JsonSerializer.SerializeObject(hashtable);
				Debug.Print(json);
				byte[] response = UTF8Encoding.UTF8.GetBytes(HtmlPageHeader + "text/json;charset=UTF-8;\r\nContent-Length: " + UTF8Encoding.UTF8.GetBytes("[" + json + "]").Length + "\r\n\r\n" + "[" + json + "]");
				ACCEPTED_SOCKET.Send(response, response.Length, 0);
		...
		/// </summary>
		/// 
		void CheckServerThread(Object state)
		{
			if (!SERVER_THREAD.IsAlive)
			{
				Debug.Print("Not alive\n try to start\n");
				//SERVER_THREAD.Abort();
				SERVER_THREAD = new Thread(new ThreadStart(RunServer));
				SERVER_THREAD.Start();
			}
		...
		{
			SERVER_THREAD.Start();
			TimerCallback timerCallback = CheckServerThread;
			timerCheckServerThread = new Timer(timerCallback, null, new TimeSpan(0, 0, 10), new TimeSpan(0, 0, 10));
		}
		/// <summary>
